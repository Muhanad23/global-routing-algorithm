...
db::RouteStatus MazeRoute::route(int startPin) {
    ...
    const double HEURISTIC_CONST1 = 0.1;
    const double HEURISTIC_CONST2 = 0.1;
    auto heuristic = [&HEURISTIC_CONST1, &HEURISTIC_CONST2](
        ...
        double part1 = std::abs(p1.x - p2.x) + std::abs(p1.y - p2.y);
        double part2= std::abs(layer1 - layer2);
        if(part1>0){
            return std::log(part1) * HEURISTIC_CONST1 +
               part2* HEURISTIC_CONST2;
        }
        else
        {
            return part1 * HEURISTIC_CONST1 +
               part2* HEURISTIC_CONST2;
        }
